...
const [, , elmModulePath] = process.argv;
...
async function optimize() {
  const originalSource = await fsp.readFile(elmModulePath, "utf8");
  const replacements = [
  ...
  await fsp.writeFile(elmModulePath, newSource);
