module Wrapper.OptimizeJs exposing (optimize)
...
applyReplacementGroup : List Optimization -> String -> Result () String
applyReplacementGroup optimizations source =
    case optimizations of
        [] ->
            Ok source

        { target, replacement } :: rest ->
            if String.contains target source then
                applyReplacementGroup
                    rest
                    (String.replace target replacement source)

            else
                Err ()


coreElmPerformanceReplacements : List Optimization
coreElmPerformanceReplacements =
