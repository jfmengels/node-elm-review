...
if (args.help) {
  ...
  [
    `init # Make your project lintable by creating a LintConfig.elm file`,
...
if (args._[0] == "init") {
  ...
  fs.copySync(lintConfigPath, path.join(process.cwd(), 'LintConfig.elm'), {overwrite: true});
  process.exit(0);
}
...
function createTemplateProject(dest) {
  ...
  pkg['source-directories'] = pkg['source-directories'].concat(path.relative(dest, process.cwd()) + '/src');
