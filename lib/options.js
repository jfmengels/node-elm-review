...
path.relative(process.cwd(), 'elm.json')
  ...
  function userSrc() {
    if (args["from-example"]) {
      // TODO Get commit from state?
      return pathToExample("ceab63e2d6d4973330ebd9eff5bda07a39defaf5");
    }

    // If using fromexample, use that
    return args.config
      ? path.resolve(process.cwd(), args.config)
      : path.join(projectToReview(), "review");
  }

  function pathToExample(commitHash /* : string */) {
    return path.join(elmStuffFolder(), "example-configurations", commitHash);
  }
  ...
  return {
    ...
    userSrc,
    fromExample: args["from-example"]
      ? parseFromExample(args["from-example"])
      : null,
    pathToExample,
    initPath,
...
function parseFromExample(string) {
  const [, userName, repoName, , branch] = /([^/]+)\/([^#]+)(#(.+))?/.exec(string);
