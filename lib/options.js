 ...
 *
 * @param {string[]} pargs - An argument array (typically `process.argv.slice(2)`)
 * @param {Path} cwd - The current working directory (typically `process.cwd()`)
 * @returns {Options | never}
 */
function compute(pargs, cwd) {
  containsHelp = pargs.includes('--help') || pargs.includes('-h');
  ...
  function projectToReview() {
    if (!elmJsonPath) {
      throw new ErrorMessage.CustomError(
        ...
        // prettier-ignore
        // TODO(@lishaduck): Remove (cwd) from message after I finish debugging.
        `I was expecting to find an ${chalk.yellowBright('elm.json')} file in the current directory (${cwd}) or one of its parents, but I did not find one.
