...
path.relative(process.cwd(), 'elm.json')
  ...
  function userSrc(templateCommit /* : string | null */) {
    if (args.template && templateCommit !== null) {
      return pathToTemplate(templateCommit);
    }
  ...
  function pathToTemplate(commitHash /* : string */) {
    return path.join(elmStuffFolder(), 'templates', commitHash);
  }
  ...
  return {
    ...
    userSrc,
    template: args.template ? parsetemplate(args.template) : null,
    pathToTemplate,
    initPath,
...
function parsetemplate(string) {
  // $FlowFixMe
  const [, userName, repoName, , reference] = /([^/]+)\/([^#]+)(#(.+))?/.exec(
    string
  );
