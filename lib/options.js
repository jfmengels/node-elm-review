...
function compute(processArgv /* : Array<string> */) {
  const args = minimist(processArgv.slice(2), {
    ...
    boolean: ['version', 'help', 'debug', 'fix', 'fix-all', 'fix-all-without-prompt', 'watch']
  });

  const elmJsonPath =
    args.elmjson ||
    args.help || // Shortcutting the search for elm.json when `--help`
    args.init || // Or `--init` are used, since we won't need it.
    findUp.sync('elm.json');
  const readmePath =
