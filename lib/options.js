...
function compute(processArgv) {
  ...
  const args = minimist(processArgv.slice(2), {
    alias: Flags.flags
      .filter(
        /** @returns {flag is Flag & {alias: string} } */
        (flag) => Boolean(flag.alias)
      )
      .reduce((object, flag) => {
    ...
    boolean: Flags.flags
      .filter(
        /** @returns {flag is Flag & {boolean: true} } */
        (flag) => flag.boolean
      )
      .map((flag) => flag.name),
    default: {
...
function checkForMissingArgs(subcommand, args) {
  Flags.flags
    .filter(
      /** @returns {flag is Flag & {boolean: false} } */
 ...
 * @returns {{gitHubUser: string | undefined, gitHubPassword: string | undefined}}
 */
