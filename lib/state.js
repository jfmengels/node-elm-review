...
module.exports = {
  ...
  useElmJsonThroughNpx,
  commitForExampleConfiguration,
  ...
  readmeChanged,
  buildRestarted,
  elmJsonIsNotAvailableThroughNpx,
  foundCommitForExampleConfiguration,
  requestReview,
  reviewFinished,
...
const model = {
  ...
  useElmJsonThroughNpx: true,
  commitForExampleConfiguration: null,
  reviewState: {
    type: 'idle'
  },
  readme: null,
...
function update(message) {
  switch (message[type]) {
    ...
    case 'elmJsonIsNotAvailableThroughNpx': {
      model.useElmJsonThroughNpx = false;
      return model;
    }

    case 'foundCommitForExampleConfiguration': {
      model.commitForExampleConfiguration = message.commit;
      return model;
    }
...
function useElmJsonThroughNpx() {
  return model.useElmJsonThroughNpx;
}

function commitForExampleConfiguration() {
  return model.commitForExampleConfiguration;
}
...
function elmJsonIsNotAvailableThroughNpx() {
  return update({
    [type]: 'elmJsonIsNotAvailableThroughNpx'
  });
}

function foundCommitForExampleConfiguration(commit /* : string */) {
  return update({
    [type]: 'foundCommitForExampleConfiguration',
    commit
  });
}
