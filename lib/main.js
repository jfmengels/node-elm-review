...
function errorHandler(err) {
  ...
  if (err instanceof errorMessage.CustomError) {
    reportCustomError(err, reviewElmJsonPath);
  } else {
...
function reportCustomError(err, reviewElmJsonPath) {
  if (options.report === 'json') {
    if (options.debug) {
      console.error(
        JSON.stringify(errorMessage.formatJson(options, err, reviewElmJsonPath), 0, 2)
      );
    } else {
      console.error(
        JSON.stringify(errorMessage.formatJson(options, err, reviewElmJsonPath))
      );
    }
  } else if (options.debug) {
    console.error(errorMessage.formatHuman(options, err));
  } else {
    console.error(errorMessage.formatHuman(options, err));
  }
}
