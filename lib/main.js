...
async function runElmReview() {
  ...
  const success = await Runner.runReview(options, app, appHash);
  AppState.exitRequested(success ? 0 : 1);
}

async function runElmReviewInWatchMode() {
  AppWrapper.stop();
  const {elmModulePath, reviewElmJson, reviewElmJsonPath, appHash} = await Builder.build(
    options
  );
  await Builder.buildElmParser(options, reviewElmJson);
  ...
  await Runner.runReview(options, initialization.app, appHash).catch(errorHandler);
}
