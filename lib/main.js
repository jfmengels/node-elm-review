...
async function runElmReviewInWatchMode() {
  const elmModule = await builder.build(options);
  const intitialization = await runner.initializeApp(options, elmModule);
  watch(options, intitialization, errorHandler);
  await runner.runReview(options, intitialization.app).catch(errorHandler);
}
