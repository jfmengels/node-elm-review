...
async function runElmReview() {
  const reviewElmJsonP = deferPromise();
  const elmModulePathP = deferPromise();
  const appHashP = deferPromise();

  const appBuildP = Builder.build(options, {
    onReviewElmJson: reviewElmJsonP.resolve,
    onElmModulePath: elmModulePathP.resolve,
    onAppHash: appHashP.resolve
  });

  console.timeEnd('build time');

  const [{app}] = await Promise.all([
    dependsOn([reviewElmJsonP], (reviewElmJson) =>
      Builder.buildElmParser(options, reviewElmJson)
    ).then(() =>
      dependsOn(
        [reviewElmJsonP, elmModulePathP, appHashP, appBuildP],
        (reviewElmJson, elmModulePath, appHash) =>
          Runner.initializeApp(options, elmModulePath, reviewElmJson, appHash)
      )
    ),

    dependsOn([appHashP], (appHash) => ResultCache.load(options, appHash))
  ]);

  if (!(await elmModulePathP.promise)) {
...
function deferPromise() {
  ...
  return {resolve: resolve_, promise};
}

function dependsOn(dataPromises, fn) {
  return Promise.all(dataPromises.map((p) => p.promise || p)).then((data) =>
    fn(...data)
  );
}
