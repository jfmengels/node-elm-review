...
async function runElmReview() {
  ...
  const {app} = await runner.initializeApp(options, elmModulePath, reviewElmJson);
...
async function runElmReviewInWatchMode() {
  ...
  const initialization = await runner.initializeApp(options, elmModulePath, reviewElmJson);
  watch(
    options,
