...
async function runElmReview() {
  ...
  const [{app}] = await Promise.all([
    reviewElmJsonP.promise
      .then((reviewElmJson) => Builder.buildElmParser(options, reviewElmJson))
      .then(() =>
        Promise.all([
          reviewElmJsonP.promise,
          elmModulePathP.promise,
          appHashP.promise,
          appBuildP
        ])
      )
      .then(([reviewElmJson, elmModulePath, appHash]) =>
        Runner.initializeApp(options, elmModulePath, reviewElmJson, appHash)
      ),

    appHashP.promise.then((appHash) => ResultCache.load(options, appHash))
  ]);
