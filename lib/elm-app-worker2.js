...
const [elmModulePath, ...reviewAppFlags] = process.argv.slice(2);
console.log(elmModulePath, reviewAppFlags);
...
const app = elmModule.Elm.Elm.Review.NodeMain.init({
  flags: {
    reviewAppFlags,
    logger: ElmCommunication.create({
      debug: reviewAppFlags.includes("--debug"),
      showBenchmark: reviewAppFlags.includes("--benchmark-info")
    })
  }
});
