...
function subscribe(options, app) {
  ...
  askConfirmationToFixAll =
    askConfirmationToFixAll || askConfirmationToFixAllWithOptions(options);
  app.ports.askConfirmationToFix.subscribe(askConfirmationToFix);
  app.ports.askConfirmationToFixAll.subscribe(askConfirmationToFixAll);
}
...
function askConfirmationToFixAllWithOptions(options) {
  return async data => {
    console.log(styledMessage(options, data.confirmationMessage));
    if (latestFixProposalAll) {
      latestFixProposalAll = data.file;
      console.log(
        `${chalk.cyan('?')} ${chalk.bold(
          'Do you wish to apply this fix?'
        )} ${chalk.gray('› (Y/n)')}`
    ...
    if (
      !latestFixProposalAll ||
      !(await checkIfAFixConfirmationIsStillExpected(currentApp))
    ) {
      latestFixProposalAll = null;
      return;
    }
    ...
    if (accepted) {
      ...
      // I could not manage to make this work at the moment
      //
      // const file = await formatFileContent(latestFixProposalAll)
      // fs.writeFileSync(file)
      fs.writeFileSync(latestFixProposalAll.path, latestFixProposalAll.source);
      const file = formatExistingFile(options, latestFixProposalAll);
      updateFileMemoryCache(options, file);
      latestFixProposalAll = null;
      currentApp.ports.userConfirmedFix.send({
        answer: true,
