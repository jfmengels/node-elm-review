...
function watch(
  ...
  const elmJsonWatcher = chokidar
    ...
    .on('change', async () => {
      ...
      if (JSON.stringify(newValue) !== JSON.stringify(elmJsonContent)) {
        ...
        debug('Your `elm.json` has changed. Restarting elm-review.');
  ...
  const readmeWatcher = chokidar
    ...
    .on('add', async () => {
      debug('README.md has been added');
    ...
    .on('change', async () => {
      ...
      if (readmeHasChanged) {
        debug('README.md has been changed');
  ...
  const fileWatcher = chokidar
    ...
    .on('add', async absolutePath => {
      ...
      debug(`File ${relativePath} has been added`);
    ...
    .on('change', async absolutePath => {
      ...
      if (elmFile.source !== newSource) {
        debug(`File ${relativePath} has been changed`);
    ...
    .on('unlink', absolutePath => {
      const relativePath = path.relative(process.cwd(), absolutePath);
      debug(`File ${relativePath} has been removed`);
  ...
  const configurationWatcher = chokidar
    ...
    .on('change', async () => {
      ...
      if (!options.debug && options.report !== 'json') {
        clearConsole();
      }
