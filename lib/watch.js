...
function watch(
  ...
  const readmeWatcher = chokidar
    .watch(removeWindowsSeparators(readme.path), {ignoreInitial: true})
    .on('add', async () => {
      if (options.debug) {
        console.log('README.md has been added');
      }

      readmeContent = await fsReadFile(readme.path, 'utf8');
      app.ports.collectReadme.send({
        path: readme.path,
    ...
    .on('change', async () => {
      const newContent = await fsReadFile(readme.path, 'utf8');
  ...
  const fileWatcher = chokidar
    ...
    .on('add', async absolutePath => {
      ...
      if (isNewFile) {
        appState.fileWasUpdated(elmFile);
      }
