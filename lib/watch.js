...
function watchFiles(
  ...
  const extraFilesWatcher = createExtraFilesWatcher(options, app, runReview, onError, requestedExtraFiles);
...
function createExtraFilesWatcher(
  ...
  return chokidar
    .watch(
      request.files.map(({pattern, included}) => (included?"":"!")+OsHelpers.makePathOsAgnostic(pattern)),
      {
