...
function watchFiles(
  ...
  const {elmJsonPath, readmePath} = options;

  const elmJsonWatcher =
    elmJsonPath &&
    chokidar
      .watch(OsHelpers.makePathOsAgnostic(elmJsonPath), {
        ignoreInitial: true
      })
      .on('change', async () => {
        const newValue = await FS.readJsonFile(elmJsonPath);
        if (JSON.stringify(newValue) !== JSON.stringify(elmJsonContent)) {
          elmJsonContent = newValue;
          runReview = () => {};
          clearTimeout(suppressedErrorsTimeout);
          await Promise.all([
            elmJsonWatcher && elmJsonWatcher.close(),
