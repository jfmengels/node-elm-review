 ...
 * @import {App, FileReceipt} from "./types/app"
 * @import {ElmFile, Readme, ElmJsonData, Dependencies, LinksToRuleDocs, NonElmFiles, ExtraFileRequest} from './types/content'
...
async function initializeApp(options, elmModulePath, reviewElmJson, appHash) {
  ...
  const requestedExtraFilesP = new Promise(
    (/** @type {(value: ExtraFileRequest[]) => void } */ r) => {
      resolve = r;
    }
  );
  const extraFilesP = requestedExtraFilesP.then(ExtraFiles.collect);
  // @ts-expect-error - `resolve` isn't assigned yet. Seems legit.
