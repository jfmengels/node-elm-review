...
function sendProjectContent(
  ...
  // TODO remove the json stringify+parse
  app.ports.collectLinks.send(
    JSON.parse(JSON.stringify(linksToDocumentationForModule))
  );
...
async function initializeApp(options, elmModulePath, reviewElmJson) {
  ...
  const {
    projectDeps,
    linksToDocumentationForModule
  } = await projectDependencies.collect(
    options,
    elmJsonData.project,
