 ...
 * @import {ApplicationElmJson, Dependencies, ElmFile, ElmJsonData, ExtraFileRequest, PackageSourcesRequest, LinksToRuleDocs, NonElmFiles, Readme} from './types/content';
...
async function initializeApp(options, elmModulePath, reviewElmJson, appHash) {
  ...
  AppState.subscribe(
    app.ports.requestNeededPackageSources,
    handlePackageSourcesRequest
  );
  ...
  /**
   *
   * @param {PackageSourcesRequest[]} requests
   */
  async function handlePackageSourcesRequest(requests) {
    const fetchedFiles = requests.map(async ({name, version, files}) => {
      const loadedFiles = files.map(async (/** @type {Path}  */ file) => {
        const path = ProjectJsonFiles.getDependencyFileFromElmHomePath(
          elmVersion,
          name,
          version,
          file
      ...
      return [name, await Promise.all(loadedFiles)];
    });

    app.ports.collectPackageSources.send(
      Object.fromEntries(await Promise.all(fetchedFiles))
    );
  }
