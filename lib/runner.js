...
function sendProjectContent(options, app, elmJsonData, elmFiles, dependencies) {
  ...
  const readme = getReadme(options);
  if (readme) {
...
async function initializeApp(options, elmModule) {
  ...
  await sendProjectContent(options, app, elmJsonData, elmFiles, projectDeps);
