...
function sendProjectContent(options, app, elmJson, elmVersion, elmFiles) {
...
function startReviewing(options, app) {
...
async function runReview(options, elmModule) {
  const {elmJson, elmFiles} = await getProjectFiles(options);
  ...
  await sendProjectContent(options, app, elmJson, elmVersion, elmFiles);
  const result = await startReviewing(options, app);
