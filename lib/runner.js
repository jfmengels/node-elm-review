...
const {getProjectFiles, writeElmFile} = require('./elm-files');
  ...
  return new Promise(resolve => {
    app.ports.askConfirmationToFix.subscribe(async data => {
      ...
      if (accepted) {
        ...
        // await writeElmFile(file)
        writeElmFile(data.file);
...
async function runLinting(args, elmModule) {
  const {elmJson, elmFiles} = await getProjectFiles(args);

  if (elmFiles.length === 0) {
    throw new Error('I could not find any files to lint.');
  }

  const app = elmModule.Elm.Elm.Lint.Main.init({
    flags: {
