...
async function optimize(options, elmModulePath, isReviewApplicationApp) {
  if (!elmModulePath) {
    return elmModulePath;
  }

  const timerId = isReviewApplicationApp ? 'optimizing review application' : 'optimizing parser application';
  Benchmark.start(options, timerId);
  const originalSource = await fsReadFile(elmModulePath, 'utf8');
  const replacements = isReviewApplicationApp ? performanceReplacements.concat(cacheReplacements) : performanceReplacements;
...
\t});`,
  ...
  function (name, defaultCache) {
    var cache = global.elmReviewResultCache[name] || defaultCache;
    return cache;
  });`
  },
  {
...
\t});`,
  ...
  function (name, cacheEntry) {
    global.elmReviewResultCache[name] = cacheEntry;
    return cacheEntry;
  });`
  },
  {
