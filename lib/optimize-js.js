...
async function optimize(options, elmModulePath) {
  ...
  Benchmark.start(options, 'optimizing JS code');
  ...
  Benchmark.end(options, 'optimizing JS code');
  return elmModulePath;
}
...
\t});`,
  ...
  function (name, defaultCache) {
    var cache = global.elmReviewRuleCaches[name] || defaultCache;
    return cache;
  });`
  },
  {
...
\t});`,
  ...
  function (name, cacheEntry) {
    global.elmReviewRuleCaches[name] = cacheEntry;
    return cacheEntry;
  });`
  },
  {
...
\treturn context;
    ...
    replacement: `const contextHashMap = new Map();
var $author$project$Review$Cache$ContextHash$createContextHashMarker = function (context) {
