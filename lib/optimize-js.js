...
async function optimize(options, elmModulePath, isReviewAppApp) {
  ...
  const replacements = isReviewAppApp
    ? performanceReplacements.concat(
  ...
  const newSource = replacements
    ...
    .replace(
      /^var ([$\w]+) = F(\d)\(\s*function\s*\((([^\n]*)\}\)\;$)?|^\t?\}\)\;$/gm,
      (match, fnName, arity, _1, _2, offset, string, ...rest) => {
        if (match.endsWith("});")) {
          ...
          if (match === "});") {
            ...
            currentFunctionName = '';
            return `}\nvar ${fnName} = F${currentArity}(${fnName}$fn);`;
          }
          const nextLines = string.slice(offset, offset + 1000).split("\n").slice(1);
          let line; 
          while ((line = nextLines.pop()) == "") {console.log(line)}
...
//           if (lineEnd) {
//             return `function ${fnName}$fn(${lineEnd}
// var ${fnName} = F${arity}(${fnName}$fn);`;
//           }
