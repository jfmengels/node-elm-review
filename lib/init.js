...
function createElmJson(args, folder) {
  const version = getElmVersion(args);
  const elmJson = {
    type: 'application',
    'source-directories': ['.'],
...
function getElmVersion(args) {
  const elmBinary = getElmBinary(args);
