...
async function saveResultFor(options, cacheFolder, cacheKey, cacheEntry) {
  await createCacheFolder(cacheFolder);
...
const cacheFolderCreationPromises = {};
function createCacheFolder(cacheFolder) {
  const resolvedPromise = cacheFolderCreationPromises[cacheFolder];
  if (resolvedPromise) {
    return resolvedPromise;
  }
  const promise = fsMkdirp(cacheFolder).catch(() => {});
  cacheFolderCreationPromises[cacheFolder] = promise;
  return promise;
}