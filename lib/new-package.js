...
async function newPackage(options) {
  ...
  const ruleName = await newRule.askForRuleName();
...
async function askForPackageName() {
  ...
  if (
    packageName.startsWith('elm-review-') ||
    packageName.startsWith('review-')
  ) {
    return packageName;
  }

  console.log(
    'The package name needs to start with "elm-review-" or "review-".'
  );
