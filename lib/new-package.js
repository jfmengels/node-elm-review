...
async function createProject(
  ...
  Spinner.succeedAndNowDo('Adding example folder');
  const pathToExample = path.join(process.cwd(), packageName, 'example');
  await Init.create(options, pathToExample, 'ReviewConfigForExample.elm', []);

  // Adding package to the example's elm.json
  const exampleElmJsonPath = path.join(pathToExample, 'elm.json');
  const exampleElmJson = fs.readJsonSync(exampleElmJsonPath);
  exampleElmJson.dependencies.direct[`${authorName}/${packageName}`] = '1.0.0';
  fs.writeFileSync(exampleElmJsonPath, JSON.stringify(exampleElmJson, null, 4));

  // Adding rule to the example's ReviewConfig
  const exampleReviewConfigPath = path.join(
    pathToExample,
    'src/ReviewConfig.elm'
  );
  const exampleReviewConfig = fs.readFileSync(exampleReviewConfigPath, 'utf8');
  fs.writeFileSync(
    exampleReviewConfigPath,
    exampleReviewConfig.replace(/RULENAME_TO_REPLACE/g, ruleName)
  );
...
function packageJson(options) {
  return {
    scripts: {
      ...
      'package-tests': 'node elm-review-package-tests/check-examples-compile.js',
      review: 'elm-review',
