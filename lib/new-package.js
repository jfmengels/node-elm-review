...
async function newPackage(options) {
  ...
  const ruleType = options.newRuleName || (await NewRule.askForRuleType(options));
  if (!ruleType) {
    return;
  }

  await createProject(options, authorName, packageName, ruleName, ruleType, license);
...
async function createProject(
  ...
  writeFile(
    ...
    NewRule.newSourceFile(`${authorName}/${packageName}`, ruleName, options.ruleType)
  );
  writeFile(
    dir,
