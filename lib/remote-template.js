...
function got2(url, options) {
  console.log("url", url)
  if (url === "https://raw.githubusercontent.com/jfmengels/elm-review-performance/13b2132c7bc51efec64bd57aa5e58a32525af9d5//preview/example/elm.json") {
    console.log("data")
  }
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(url, options, (res) => {
      ...
      if (res.statusCode === 301) {
        ...
        urlObject.pathname = res.headers.location;
        return got2(urlObject.toString(), options).then(resolve).catch(reject);
      }
      ...
      res.on('end', () => {
        ...
        return reject({statusCode: res.statusCode, body: JSON.parse(data)});
      });
    });
...
const gitHubApiHeaders = {
  'User-Agent': 'jfmengels/elm-review'
};
...
function makeGitHubApiRequest(options, url, handleNotFound) {
  ...
  if (options.gitHubUser && options.gitHubPassword) {
    parameters.auth = `${options.gitHubUser}:${options.gitHubPassword}`;
  }

  Debug.log(`Making API request to GitHub: ${url}`);
  return got2(url, parameters)
    // .then((body) => console.log(body) || body)
    .then((body) => JSON.parse(body))
    .catch((error) => {
      Debug.log(`An error occurred when making a request to the GitHub API:`);
      Debug.log(`# url: ${url}`);
      Debug.log(`# statusCode: ${error.statusCode}`);
      Debug.log(`# body:\n${JSON.stringify(error.body, 0, 4)}`);
