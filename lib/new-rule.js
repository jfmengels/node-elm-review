...
async function addRule(options, elmJson, ruleName) {
  ...
  injectRuleInExample(dir, elmJson, ruleName);
}

function injectRuleInExample(dir, elmJson, ruleName) {
  try {
    const content = fs.readFileSync(
      path.join(dir, 'example/src/ReviewConfig.elm'),
      'utf8'
    );
    const lines = content.split('\n');
    insertRuleInConfiguration(elmJson, ruleName, lines);
    writeFile(dir, 'example/src/ReviewConfig.elm', lines.join('\n'));
  } catch (_) {
    console.log(
      `${chalk.red('[WARNING]')} Could not find ${chalk.yellow(
        'example/src/ReviewConfig.elm'
      )}`
    );
  }
}
...
function insertImport(ruleName, lines) {
  ...
  if (firstImportIndex === -1) {
    console.log(
      `${chalk.red(
        '[WARNING]'
      )} Could not find where to add an import of the rule in the example configuration`
    );
    return;
  }
