...
async function addRule(options, elmJson, ruleName) {
  ...
  injectRuleInPreview(dir, elmJson, ruleName);
}

function injectRuleInPreview(dir, elmJson, ruleName) {
  try {
    const content = fs.readFileSync(
      path.join(dir, 'preview/src/ReviewConfig.elm'),
      'utf8'
    );
    const lines = content.split('\n');
    insertRuleInConfiguration(elmJson, ruleName, lines);
    writeFile(dir, 'preview/src/ReviewConfig.elm', lines.join('\n'));
  } catch {
    console.log(
      `${chalk.red('[WARNING]')} Could not find ${chalk.yellow(
        'preview/src/ReviewConfig.elm'
      )}`
    );
  }
}
