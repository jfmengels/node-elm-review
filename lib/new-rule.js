  ...
  const {ruleType} = await prompts(
    {
      type: 'select',
      name: 'ruleType',
      message: `Choose the kind of rule you want to start with:`,
      choices: [
        { title: 'Module rule', value: 'module', description: 'Simpler, but looks at modules in isolation' },
        { title: 'Project rule', value: 'project', description: 'More complex, but can access a lot more information' },
      ],
      initial: 0
    },
    {
...
-}
${ruleType === 'project' ? projectRuleTemplate(ruleName) : moduleRuleTemplate(ruleName)}`;
}
