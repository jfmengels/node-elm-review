...
function styleMessage(colorEnabled, message) {
  return message
    .map((part) => {
      const chalkWithColor = colorEnabled ? withStyling(part) : chalk;
      const string = chalkWithColor(typeof part === 'string' ? part : part.str);
      if (colorEnabled && part.href && terminalLink.isSupported) {
        return terminalLink(string, part.href);
      }
