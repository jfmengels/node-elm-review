...
async function collect(options, elmJson, elmVersion) {
  ...
  let dependenciesThatCouldNotBeDownloaded = [];
    ...
    async ([pkgName, constraint]) => {
      ...
      const [docsJson, dependencyElmJson] = await Promise.all([
        ProjectJsonFiles.getDocsJson(
          ...
          dependenciesThatCouldNotBeDownloaded.push(`- ${name} ${packageVersion}`);
          return [];
        }),
  ...
  if (dependenciesThatCouldNotBeDownloaded.length > 0 && options.report !== 'json') {
    console.log(
      `
I could not fetch all the data I need about these dependencies of your project:
${[...new Set(dependenciesThatCouldNotBeDownloaded)].join('\n')}
