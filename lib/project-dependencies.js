  ...
  const dependenciesInfo = await Promise.all(
    Object.entries(dependenciesEntries).map(async ([name, constraint]) => {
      ...
      const [docsJson, dependencyElmJson] = await Promise.all([
        fsReadJson(path.join(directory, 'docs.json'))
          .catch(() => readDocsJsonFromPackagesWebsite(name, packageVersion))
          .catch(() => {
            hasDependenciesThatCouldNotBeDownloaded = true;
            return [];
          }),
        fsReadJson(path.join(directory, 'elm.json'))
          .catch(() => readElmJsonFromPackagesWebsite(name, packageVersion))
          .catch(() => {
            hasDependenciesThatCouldNotBeDownloaded = true;
