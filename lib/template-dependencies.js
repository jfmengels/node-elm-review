...
async function getFromCacheOrCompute(folder, key, fn) {
  const filepath = path.join(folder, `${key}.json`);

  let cachedResult = await fsReadJson(filepath).catch(() => null);
  if (cachedResult) {
    return cachedResult;
  }
...
async function get(options, elmJsonDependencies, pathToElmJson) {
  ...
  const cacheKey = `${dependencyHash}${options.localElmReviewSrc ? '-local' : ''}`;
  return getFromCacheOrCompute(options.dependenciesCachePath(), cacheKey, () => computeDependencies(options, pathToElmJson));
}
