...
async function get(options, elmVersion, elmJsonDependencies, pathToElmJson) {
  ...
  return Cache.getOrCompute(options.dependenciesCachePath(), cacheKey, () =>
    computeDependencies(options, elmVersion, pathToElmJson)
  );
}

function computeDependencies(options, elmVersion, pathToElmJson) {
...
function solve(options, elmVersion, elmJson, extra) {
    ...
    try {
        return dependencyProvider.solveOffline(elmJson, extra);
    } catch (error) {
        if (options.offline) {
            throw error;
        }
        return dependencyProvider.solveOnline(elmJson, extra);
    }
}
