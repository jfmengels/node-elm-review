module.exports = {
  stringify: (options, data) => JSON.stringify(data, replacer(options), options.debug ? 2 : 0),
  parse: (options, data) => JSON.stringify(data, reviver(options), options.debug ? 2 : 0),
  replacer,
  reviver
}
...
const optimizedData = {"b6":"b","bc":{"$":-1,"a":1,"b":"dictOfThings2","c":12,"d":{"$":-1,"a":1,"b":"dictOfThings","c":123456789,"d":{"$":-2},"e":{"$":-2}},"e":{"$":-1,"a":1,"b":"dictOfThings3","c":90,"d":{"$":-2},"e":{"$":-2}}},"da":{"$":-2},"aI":{"$":"LIST","a":[{"a$":{"$":0,"a":"src/Page/Blank.elm"},"C":{"$":0},"cn":{"$":"LIST","a":["Page","Blank"]},"ax":{"$":"LIST","a":[{"aK":true,"a6":"view"}]}}]},"cU":{"$":0},"aQ":{"$":1}}
