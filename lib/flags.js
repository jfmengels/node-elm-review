...
const templateFlag = {
  ...
  description: [
    ...
    '  - elm-review --template jfmengels/elm-review-unused/example#main',
    '',
    // prettier-ignore
  ...
  initDescription: [
    ...
    '- elm-review init --template jfmengels/elm-review-unused/example#main'
  ]
};
...
function buildFlag(subcommand, flag) {
  ...
  const alias =
    flag.name === 'version' ? ', -v' : flag.name === 'help' ? ', -h' : '';
  return `    ${flag.color(`--${flag.name}${alias}${flagArgs}`)}
${description
  .map(
    /**
     * @param {string} desc
