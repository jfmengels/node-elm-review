...
const tasks = {
  ...
  "fs:writeBytes": (args) =>
    fs
      .writeFile(args.path, Buffer.from(args.content, "base64"))
      .catch(wrapError),
  "fs:stat": (args) =>
    fs
      .stat(args.path)
  ...
  "fs:createSymlink": (args) =>
    fs.symlink(args.target, args.linkPath).catch(wrapError),
  "fs:createDirectory": (path) =>
    fs.mkdir(path, {recursive: true}).catch(wrapError),
  "fs:removeDirectory": removeDirectory,
  ...
  "os:killProcess": ({pid, signal}) =>
    killProcess(pid, signal).catch(wrapError),

  "std::println": println,
  "std::readLine": () => readline_().catch(wrapError),
  "std::readKey": () => readKey().catch(wrapError)
};
...
async function killProcess(pid, signal) {
  ...
  if (spawnResult) {
    spawnResult.spawned.kill(signal);
  }
  return null;
}
