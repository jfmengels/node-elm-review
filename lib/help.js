...
You can customize the linting command with the following flags:
    ...
    ${chalk.cyan('--extra <path-to-directory-to-lint>')}
        Add another directory to lint. Use this if you want to lint directories
        like ${chalk.blue('tests/')} or ${chalk.blue('lint/')}.
        Repeat the flag if you wish to add multiple directories.

    ${chalk.cyan('--lint-folder <path-to-lint-directory>')}
...
If you wish to use ${chalk.blueBright('automatic fixing')}, you can use the following flags:
    ...
    ${chalk.blueBright('--fix')}
        ${chalk.greenBright('elm-lint')} will present you with fixes for the errors that offer an
        automatic fix, which you can then accept or not. When there are no more
        fixable errors left, ${chalk.greenBright('elm-lint')} will report the remaining errors as if it
        was called without ${chalk.blueBright('--fix')}.
