...
function suppress() {
  ...
  /* eslint-disable prettier/prettier */
  console.log(`The suppress command helps adding new rules that report many errors in your project.
...
First is the use of the ${chalk.cyan('Rule.ignoreErrorsForDirectories')} and ${chalk.cyan('Rule.ignoreErrorsForFiles')} functions in the ${chalk.cyan('ReviewConfig.elm')} file. These aim to indicate the parts of the project where a rule would not provide value and should be ignored, such as generated or vendored code.

The second system is the ${orange('temporarily suppressed')} errors system, which aims to help you gradually adopt rules that report many errors in your project without having you fix all the issues beforehand. It will do so by ignoring the currently reported errors, reporting only the new errors, yet reminding you about the errors that need to be fixed.

Running ${orange('elm-review suppress')} will generate JSON files in ${orange('review/suppressed/')}. Each file corresponds to a rule, which records the number of suppressed errors per file in your project.

While suppression files exist for your project, running ${chalk.greenBright('elm-review')} will behave as usual but with these additional behaviors:
...
While you can run the ${orange('suppress')} command to ignore newly reported errors, please do so with moderation. The aim of the suppression system is to allow enabling rules while there are errors remaining and to have these fixed incrementally, not to make it easier to ignore errors.
