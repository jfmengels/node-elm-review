...
function review(options) {
  ...
  console.log(`You are using ${chalk.greenBright(`elm-review ${options.packageJsonVersion}`)}.
    ...
    ${chalk.yellowBright('elm-review init')}
        Creates an empty review configuration. It creates a ${chalk.cyan('review/elm.json')} and
        a ${chalk.cyan('review/ReviewConfig.elm')} file, and provides links to help you get
...
If you wish to use ${chalk.blueBright('automatic fixing')}, you can use the following flags:
    ...
    ${chalk.blueBright('--fix')}
        ...
        fix, which you can then accept or refuse. When there are no more fixable
        errors left, ${chalk.greenBright('elm-review')} will report the remaining errors as if it was
        called without ${chalk.blueBright('--fix')}.
