...
function getElmFiles(args, elmJson) {
  if (args.debug) {
    console.log('Linting the following files:');
  }

  return Promise.all(
    [...new Set(getFilesToLint(elmJson))].map(async filePath => {
      if (args.debug) {
...
function getFilesToLint(elmJson) {
  if (elmJson.type === 'package') {
    return glob.sync('src/**/*.elm', {
      root: process.cwd(),
      nocase: true,
      ignore,
      nodir: false
  ...
  return flatMap(elmJson['source-directories'], directory => {
    return glob
      .sync(path.resolve(process.cwd(), directory) + '/**/*.elm', {
        nocase: true,
        ignore,
        nodir: false
      })
