...
function getFilesToLint(projectToLint, directoriesFromCLIArguments, elmJson) {
  if (directoriesFromCLIArguments.length !== 0) {
    return directoriesFromCLIArguments.map(directory => {
      // TODO Performance: Call these globs in parallel
      return {
        files: glob.sync(path.resolve(process.cwd(), directory) + '/**/*.elm', {
          nocase: true,
          ignore,
          nodir: false
        }),
  ...
  if (elmJson.type === 'package') {
    return [
      {
        files: glob.sync(path.resolve(projectToLint, 'src') + '/**/*.elm', {
          nocase: true,
          ignore,
          nodir: false
        }),
        directory: path.resolve(projectToLint, 'src')
      }
    ];
  }
  ...
  return elmJson['source-directories'].map(directory => ({
    files: glob.sync(directory + '/**/*.elm', {
      nocase: true,
      ignore,
      nodir: false
    }),
    directory: path.resolve(projectToLint, directory)
  }));
}
