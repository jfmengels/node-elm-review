...
async function getProjectFiles(args, projectToLint) {
  ...
  const elmFiles = await Promise.all(
    [...new Set(getFilesToLint(projectToLint, args._, elmJson))].map(
      async filePath => {
        if (args.debug) {
...
function getFilesToLint(projectToLint, directoriesFromCLIArguments, elmJson) {
  if (directoriesFromCLIArguments.length !== 0) {
    return filesInDirectories(directoriesFromCLIArguments);
  }
...
const filesInDirectories = directories => {
  return flatMap(directories, directory => {
    // TODO Performance: Call these globs in parallel
    return glob.sync(path.resolve(process.cwd(), directory) + '/**/*.elm', {
      nocase: true,
      ignore,
      nodir: false
    });
