...
async function readFile(
  ...
  const relativeFilePath = PathHelpers.toUnixPath(
    path.relative(relativePathToElmJson, filePath)
  );
...
async function getReadme(options, directoryContainingElmJson) {
  try {
    ...
    return {
      path: PathHelpers.toUnixPath(
        path.relative(directoryContainingElmJson, options.readmePath)
      ),
      content
    };
  } catch {
