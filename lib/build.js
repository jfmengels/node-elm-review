...
async function buildFromGitHubExample(options) {
  const commit = await getLatestCommitForBranch('master').catch(() =>
    getLatestCommitForBranch('main')
  );
...
async function getLatestCommitForBranch(branch) {
  const {
    ...
    `https://api.github.com/repos/jfmengels/review-unused/branches/example`,
    {responseType: 'json'}
  );
