...
async function cachedBuild(userSrc, reviewElmJsonPath, reviewElmJson) {
...
function compileElmProject(options, dest, binaryPath, elmBinary) {
  const compileOptions = {
    ...
    processOpts: {
      env: process.env,
      // TODO Find a way to get compiler errors to be colored, just when
      // we were using `stdio: ['ignore', 'ignore', 'inherit']`
  ...
  return new Promise((resolve, reject) => {
    ...
    compileProcess.stdio[2].on('data', data => {
      stderrBuffer = Buffer.concat([stderrBuffer, data]);
      if (data.indexOf('-- MISS') >= 0) {
        reject(
          new Error(
            /* eslint-disable prettier/prettier */
            errorMessage('MISSING INTERNET ACCESS',
