...
function build(options, dataResolvers) {
  if (options.template) {
    return buildFromGitHubTemplate(options, dataResolvers);
  }

  return buildLocalProject(options, dataResolvers);
}

async function buildLocalProject(options, dataResolvers) {
...
options.elmJsonPath
  ...
  dataResolvers.onReviewElmJson(reviewElmJson);

  validateElmReviewVersion(options, reviewElmJsonPath, reviewElmJson);

  Benchmark.start(options, 'Calculating app hash');
  ...
  Benchmark.end(options, 'Calculating app hash');
  dataResolvers.onAppHash(appHash);
  ...
  dataResolvers.onElmModulePath(elmModulePath);
...
async function buildFromGitHubTemplate(options, dataResolvers) {
  ...
  dataResolvers.onAppHash(commit);
