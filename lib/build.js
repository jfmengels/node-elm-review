  ...
  const compileOptions = {
    ...
    processOpts: {
      ...
      stdio: [
        'ignore',
        'ignore',
        // TODO We should be able to get the error report and give it back.
        // At least in JSON mode
        options.report === 'json' ? 'ignore' : 'inherit'
      ]
    }
  };
...
function validateElmReviewVersion(reviewElmJsonPath, elmReviewVersion) {
  if (!elmReviewVersion) {
    throw new Error(
      /* eslint-disable prettier/prettier */
      errorMessage.create(
...
of ${path.dirname(reviewElmJsonPath)}.`
  ...
  if (major === '2' && minor < parseInt('1', 10)) {
...
${chalk.cyanBright('npx elm-json install jfmengels/elm-review@1')} inside
of ${path.dirname(reviewElmJsonPath)}.`
      /* eslint-enable prettier/prettier */
    );
  }
