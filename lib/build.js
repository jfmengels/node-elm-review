...
const ElmOptimizeLevel2 = require('/home/jeroen/dev/elm-optimize-level-2');
...
function compileElmProject(
  ...
  Benchmark.start(options, 'Elm compilation');
  return new Promise((resolve) => {
    ...
    Benchmark.end(options, 'Elm compilation');
    return optimize(options, elmModulePath);
  });
}
...
async function optimize(options, elmModulePath) {
  ...
  Benchmark.start(options, 'Applying elm-optimize-level-2');
  const elmOptimizeOutput = await ElmOptimizeLevel2.run({
    inputFilePath: elmModulePath,
    outputFilePath: elmModulePath,
    optimizeSpeed: true,
    processOpts: undefined
  });
  Benchmark.end(options, 'Applying elm-optimize-level-2');
  return elmModulePath;
}
