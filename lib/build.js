...
async function buildFromGitHubTemplate(options) {
  const {userSrc, reviewElmJson} = await downloadTemplate(options);
  const reviewElmJsonPath = path.join(userSrc, 'elm.json');
...
async function downloadTemplate(options) {
  const {reference} = options.template;

  const commit = await (reference
    ? getLatestCommitForReference(options, reference)
  ...
  const elmJson = JSON.parse(rawElmJson);
  const pathToTemplate = options.pathToTemplate(commit);
  await fsMkdirp(pathToTemplate);
  await fsWriteFile(path.join(pathToTemplate, 'elm.json'), rawElmJson);
  await Promise.all(
