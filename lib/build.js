...
function compilationError(options, stderr) {
  ...
  // TODO Add title and all messages
  const compilationErrors =
    options.report === 'json'
      ? JSON.parse(stderr).errors[0].problems[0].message
      : [];

  return {
    title: 'CONFIGURATION COMPILATION ERROR',
    // TODO Support having message be an array
    message: [
      'An error occurred while compiling your configuration for ',
      {
        string: 'elm-review',
        color: '#00FF00'
      },
      '. I need your configuration to compile in order to know how to analyze your files. Hopefully the compiler error below will help you figure out how to fix it.',
      ...compilationErrors
    ]
  };
}
