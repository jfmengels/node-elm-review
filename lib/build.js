...
async function buildFromGitHubTemplate(options) {
  ...
  const commit = await (branch
    ? getLatestCommitForBranch(userName, repoName, branch).catch(() => branch)
...
async function findDefaultBranch(userName, repoName) {
  const {body} = await got(
    ...
    handleGitHubApiErrors(() => repoNotFoundErrorMessage(userName, repoName))
  );
  ...
  return (error) => {
    if (error.name === 'HTTPError') {
      switch (error.response.statusCode) {
        case 404:
          throw new errorMessage.CustomError(...handle404());
