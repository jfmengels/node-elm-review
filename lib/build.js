 ...
 * @import {ElmRunOptions} from './types/elm-run';
...
I can help set you up with an initial configuration if you run ${chalk.magenta('elm-review init')}.`,
      ...
      if (error.code === 'ENOTDIR') {
        throw new ErrorMessage.CustomError(
          ...
          'INCORRECT CONFIGURATION',
          // prettier-ignore
          `I was expecting the configuration to be a path to a directory containing an ${chalk.yellow('elm.json')} file and a ${chalk.cyan('ReviewConfig.elm')} file, but it seems ${chalk.red('you pointed at a file')}.

Instead of ${chalk.red('--config some/path/to/review/elm.json')} (or similar),
please use ${chalk.greenBright('--config some/path/to/review')}`,
          options.elmJsonPath
        );
      }
  ...
  if (!options.forceBuild && fs.existsSync(elmModulePath)) {
    ...
    const [elmRunBinary] = await Promise.all([
      ElmRun.getElmRunBinary(options.cwd),
    ...
    const buildResult = await compileElmProjectUsingElmRun(
      options,
      buildFolder,
      elmModulePath,
...
function createSymLinkForLocalElmReview(options, buildFolder, reviewElmJson) {
  ...
  const version =
    ...
    reviewElmJson.dependencies.indirect['jfmengels/elm-review'];
  const packagePath = path.dirname(
    ProjectJsonFiles.getElmJsonFromElmHomePath(
...
async function createTemplateProject(
  ...
  const templateSymlinkSrc = path.join(projectFolder, 'src', 'Elm');
  ...
  const finalElmJson = updateSourceDirectories(userSrc, {
    ...reviewElmJson,
    dependencies,
  ...
  if (previousElmJson !== finalElmJsonAsString) {
    ...
    await FS.mkdirp(path.resolve(projectFolder, 'src'));
    // TODO Support Windows
    execSync(`cp -R ${path.resolve(path.join(templateSrc, 'Elm'))} ${templateSymlinkSrc}/`, {
      stdio: 'pipe',
    })
    const writeFileP = await FS.writeFile(elmJsonPath, finalElmJsonAsString);
    // FS.symlink(path.resolve(path.join(templateSrc, 'Elm')), templateSymlinkSrc, 'dir');
    await writeFileP;
  }
 ...
 */
function updateSourceDirectories(userSrc, elmJson) {
...
async function compileElmProject(
  ...
  await FS.mkdirp(path.dirname(compileOptions.output));
  /** @type {string | null} */
 ...
 * @param {Path} elmModulePath
 * @param {Path} target
 * @param {Path} elmRunBinary
 * @param {boolean} isReviewApp
 * @returns {Promise<string | null>}
 */
async function compileElmProjectUsingElmRun(
  options,
  dest,
  elmModulePath,
  target,
  ...
  const compileOptions = {
    ...
    output: path.resolve(elmModulePath),
    target,
    verbose: Boolean(options.debug),
    pathToElmRun: elmRunBinary,
    processOpts: {
      env: process.env,
  ...
  return await new Promise((resolve) => {
    const compileProcess = ElmRun.compile(compileOptions);
