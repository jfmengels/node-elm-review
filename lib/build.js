...
async function buildFromGitHubTemplate(options) {
  const {userName, repoName, branch} = options.template;
  const commit = await (branch
    ? getLatestCommitForBranch(userName, repoName, branch)
    : findDefaultBranch(userName, repoName).then((defaultBranch) =>
        getLatestCommitForBranch(userName, repoName, defaultBranch)
      ));
  ...
  // TODO Give a nice error message if the file could not be found
  ...
  await Promise.all(
    elmJson['source-directories'].map((sourceDirectory) =>
      downloadDirectory(userName, repoName, pathToTemplate, sourceDirectory)
    )
  );
}

async function findDefaultBranch(userName, repoName) {
  const {body} = await got(
...
async function getLatestCommitForBranch(userName, repoName, branch) {
  const {
    body
  } = await got(
...
async function downloadDirectory(userName, repoName, basePath, directory) {
  await fsMkdirp(path.join(basePath, directory));
  const {body: fileListing} = await got(
    // TODO Replace ref?
    `https://api.github.com/repos/${userName}/${repoName}/contents/example/${directory}?ref=example`,
    {responseType: 'json'}
  );
  ...
  await Promise.all(
    fileListing.map((fileOrDir) => {
      ...
      return downloadDirectory(basePath, path.join(directory, fileOrDir.name));
    })
  );
}
...
function compilationError(options, stderr) {
  ...
  if (stderr.includes('MODULE NOT FOUND')) {
    return {
      ...
      /* eslint-disable prettier/prettier */
      message: `A module is missing in your configuration. Maybe you forgot to add some dependencies that contain the rules you wished to enable? If so, run ${chalk.cyanBright('elm install')} with the package name from inside ${chalk.yellowBright(options.userSrc(optionalTemplateCommit))}.`
      /* eslint-enable prettier/prettier */
    };
  }
