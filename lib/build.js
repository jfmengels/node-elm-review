...
async function buildFromGitHubTemplate(options) {
  ...
  const commit = await (reference
    ? getLatestCommitForReference(userName, repoName, reference)
  ...
  return (error) => {
    if (error.name === 'HTTPError') {
      switch (error.response.statusCode) {
        ...
        case 404:
        case 422:
          throw new errorMessage.CustomError(...handle404(error.response.body));
...
function getLatestCommitForReference(userName, repoName, reference) {
  return got(
