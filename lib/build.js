...
async function build(options) {
  if (options.fromExample) {
    await buildFromGitHubExample(options);
  }
...
async function buildFromGitHubExample(options) {
  const {userName, repoName, branch} = options.fromExample;
  ...
  appState.foundCommitForExampleConfiguration(commit);
  ...
  const pathToExample = options.pathToExample(commit);
  await fsMkdirp(pathToExample);
  await fsWriteFile(path.join(pathToExample, 'elm.json'), body);
  await Promise.all(
    elmJson['source-directories'].map((sourceDirectory) =>
      downloadDirectory(userName, repoName, pathToExample, sourceDirectory)
    )
  );
}
