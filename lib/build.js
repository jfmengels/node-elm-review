...
async function buildFromGitHubTemplate(options) {
  ...
  const commit = await (branch
    ? getLatestCommitForReference(userName, repoName, branch)
    : findDefaultBranch(userName, repoName).then((defaultBranch) =>
        getLatestCommitForReference(userName, repoName, defaultBranch)
      ));
...
async function findDefaultBranch(userName, repoName) {
  const {body} = await got(
    ...
    handleGitHubApiErrors(() => ({
      message: repoNotFoundErrorMessage(userName, repoName)
    }))
  );
...
function handleGitHubApiErrors(handle404) {
  return (error) => {
    if (error.name === 'HTTPError') {
      switch (error.response.statusCode) {
        case 404: {
          const handled404Result = handle404(error.response.body);
          if (handled404Result.result) {
            return handled404Result.result;
          }

          throw new errorMessage.CustomError(...handled404Result.message);
        }
...
async function getLatestCommitForReference(userName, repoName, reference) {
  const {
    body
  } = await got(
    `https://api.github.com/repos/${userName}/${repoName}/branches/${reference}`,
    {responseType: 'json'}
  ).catch(
    handleGitHubApiErrors((responseBody) => {
      ...
      // The branch was not found, so it's likely the reference actually links
      // to a commit. To avoid hitting the API rate limit, we'll assume it's
      // correct and move on.
      return {
        result: reference
      };
