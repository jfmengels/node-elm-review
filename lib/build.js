...
async function buildFromGitHubExample(options) {
  // TODO Fetch https://api.github.com/repos/jfmengels/review-unused/contents/example?ref=example
  // first, and cache by commit?
  const {body} = await got(
    `https://raw.githubusercontent.com/jfmengels/review-unused/example/example/elm.json`
  );
  // TODO Fail if elmJson contains paths to '../xyz', that will make things easier.
  const elmJson = JSON.parse(body);
  const commit = 'TODO-fake-commit';
  ...
  await Promise.all(
    elmJson['source-directories'].map((sourceDirectory) =>
      downloadDirectory(pathToExample, sourceDirectory)
    )
  );
}

async function downloadDirectory(basePath, directory) {
  await fsMkdirp(path.join(basePath, directory));
  const {body} = await got(
    `https://api.github.com/repos/jfmengels/review-unused/contents/example/${directory}?ref=example`
  );
  const fileListing = JSON.parse(body);
  await Promise.all(
