...
async function buildLocalProject(options) {
  // TODO Remove this function and all things related
...
async function buildFromGitHubTemplate(options) {
  ...
  if (fs.existsSync(elmModulePath)) {
    ...
    const {userSrc} = await downloadTemplate(options, commit, reviewElmJson);
    const reviewElmJsonPath = path.join(userSrc, 'elm.json');
...
function getRelevantCommit(options) {
  return options.template.reference
    ...
    : findDefaultBranch(options).then((defaultBranch) =>
        getLatestCommitForReference(options, defaultBranch)
      );
}
...
async function getRemoteElmJson(options, commit) {
  ...
  // TODO Fail (here) if elm-review version is absent or outdated
  // TODO Fail if elmJson contains paths to '../xyz', that will make things easier
  return JSON.parse(rawElmJson);
}

async function downloadTemplate(options, commit, reviewElmJson) {
  // TODO Remove this function and all things using it
  appState.foundCommitForTemplate(commit);

  // Have pathToTemplate be an argument and have function not return anything
  const pathToTemplate = options.pathToTemplate(commit);
  await Promise.all(
    reviewElmJson['source-directories'].map((sourceDirectory) =>
      downloadDirectory(options, commit, pathToTemplate, sourceDirectory)
    )
  );

  return {
    userSrc: pathToTemplate
  };
}

async function downloadTemplateElmJson(options, commit) {
  const {userName, repoName, pathToFolder} = options.template;

  return got(
    `https://raw.githubusercontent.com/${userName}/${repoName}/${commit}/${pathToFolder}/elm.json`
  )
    .catch((error) => {
...
async function findDefaultBranch(options) {
  const {userName, repoName} = options.template;
...
async function downloadDirectory(options, commit, basePath, directory) {
  ...
  const fileListing = await makeGitHubApiRequest(
    options,
    `https://api.github.com/repos/${userName}/${repoName}/contents${pathToFolder}/${directory}?ref=${commit}`
  );
