...
`I could not find a review configuration. I was expecting to find an ${chalk.green('elm.json')} file and a ${chalk.cyan('ReviewConfig.elm')} file in ${chalk.cyan(path.dirname(reviewElmJsonPath))}${chalk.cyan('/')}.
...
function compilationError(options, stderr) {
  ...
  if (stderr.includes('MODULE NOT FOUND')) {
    return {
      ...
      /* eslint-disable prettier/prettier */
      message: `A module is missing in your configuration. Maybe you forgot to add some dependencies that contain the rules you wished to enable? If so, run ${chalk.magenta('elm install')} with the package name from inside ${chalk.green(options.userSrc(null))}.`
      /* eslint-enable prettier/prettier */
    };
  }
